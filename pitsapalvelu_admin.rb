...
class Pitsapalvelu_admin < Sinatra::Base
  ...
  def hae_idt
    tuote = {}
    $db.execute("select * from tuote;").each { |t|
      tuote[t['id']]=t['nimi']
    }
    lisuke = {}
    $db.execute("select * from lisuke;").each { |l|
      lisuke[l['id']]=l['nimi']
    }
    ruokalista = {}
    $db.execute("select * from ruokalista;").each { |r|
      ruokalista[r['id']]=r['nimi']
    }
	return {:tuote=>tuote, :lisuke=>lisuke, :ruokalista=>ruokalista}
  end
